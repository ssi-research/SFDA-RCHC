"""
 This file was copied from https://github.com/tim-learn/SHOT-plus/code/uda/image_mixmatch.py and modified for this project needs.
...
import utils
import networks
from data_loader import ImageList, ImageList_twice
...
def split_target(args):
    ...
    test_set = ImageList(txt_tar, transform=test_transform, img_root_dir=args.data_root_path)
    ...
    if args.net[0:3] == 'res':
        ...
        netF = networks.VGGBase(vgg_name=args.net).cuda()
    ...
    if args.model == "source":
        ...
        dict_to_load = torch.load(natsorted(glob(os.path.join(args.output_tar, 'last*.pth')))[-1])
        for component in dict_to_load:
            if component == 'M':
                netF.load_state_dict(dict_to_load[component])
            elif component == 'E':
                netB.load_state_dict(dict_to_load[component])
            elif component=='G':
                netC.load_state_dict(dict_to_load[component])
    ...
    if args.choice == "ent":
        ...
        pred, _ = torch.sort(all_output, 1, descending=True)
        value = pred[:,1] - pred[:,0]
    elif args.choice == "maxp_softmax":
        t = nn.Softmax(dim=1)(all_output)
        top_pred, predict = torch.max(t, 1)
        value = - top_pred
    elif args.choice == "marginp_softmax":
        t = nn.Softmax(dim=1)(all_output)
        pred, _ = torch.sort(t, 1, descending=True)
        value = pred[:,1] - pred[:,0]
    else:
...
def data_load(args, txt_src, txt_tgt):
    ...
    dsets = {}
    dsets["source"] = ImageList(txt_src, transform=train_transform, img_root_dir=args.data_root_path)
    dsets["target"] = ImageList_twice(txt_tgt, transform=[train_transform, train_transform], img_root_dir=args.data_root_path)

    txt_test = open(args.test_dset_path).readlines()
    dsets["test"] = ImageList(txt_test, transform=test_transform, img_root_dir=args.data_root_path)
...
def cal_acc(loader, model, flag=True):
    ...
    if flag:
        ...
        str_acc = ' '.join(aa)
        return aacc, predict, all_output, all_label, str_acc, acc
    else:
...
def train(args, txt_src, txt_tgt):
    ...
    ## set base network
    if args.net[0:3] == 'res':
        netG = networks.ResBase(res_name=args.net).cuda()
    elif args.net[0:3] == 'vgg':
        netG = networks.VGGBase(vgg_name=args.net).cuda()

    max_len = max(len(dset_loaders["source"]), len(dset_loaders["target"]))
    max_iter = args.max_epoch*max_len
    interval_iter = max_iter // 10

    netB = networks.feat_bootleneck(type=args.classifier, feature_dim=netG.in_features, bottleneck_dim=args.bottleneck).cuda()
    netC = networks.feat_classifier(type=args.layer, class_num = args.class_num, bottleneck_dim=args.bottleneck).cuda()
    ...
    if args.model == "source":
        ...
        for component in dict_to_load:
            if component == 'M':
                netG.load_state_dict(dict_to_load[component])
            elif component == 'E':
                netB.load_state_dict(dict_to_load[component])
    ...
    for iter_num in range(1, max_iter + 1):
        ...
        try:
            inputs_source, labels_source,_ ,_ = source_loader_iter.next()
        except:
            source_loader_iter = iter(dset_loaders["source"])
            inputs_source, labels_source,_ ,_ = source_loader_iter.next()
        try:
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            base_network.eval()
            if args.dset == 'VisDA-C' or args.dset == 'DomainNet':
                acc, py, score, y, tacc, tacc_all = cal_acc(dset_loaders["test"], base_network, flag=True)
                print(tacc)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--net', type=str, default='resnet50', choices=["resnet18", "resnet50", "resnet101", "resnet34", "vgg16", "R50_VIT"])
    parser.add_argument('--dset', type=str, default='OfficeHome', choices=['VisDA-C', 'office', 'OfficeHome', 'DomainNet'], help="The dataset or source dataset used")
    ...
    parser.add_argument('--choice', type=str, default="ent", choices=["maxp", "ent", "marginp", "random", "marginp_softmax", "maxp_softmax"])
    ...
    parser.add_argument('--data_root_path', type=str, default="/local_datasets/da_datasets/")
    ...
    if args.dset== 'DomainNet':
        if args.class_num==40:
            args.t_dset_path = './Data/' + args.dset + '/' + names[args.t] + '_train_mini.txt'
            args.test_dset_path = './Data/' + args.dset + '/' + names[args.t] + '_test_mini.txt'
    else:
